...
class ClinicLabManagementSystem:
    ...
    def create_add_patient_frame(self):
        ...
        frame_add_patient = tk.Frame(self.window) 
        frame_add_patient.pack(pady=20)
        ...
        btn_add_patient = tk.Button(frame_add_patient, text="Add Patient", command=self.add_patient , bg="purple",fg="white")
    ...
    def create_add_test_frame(self):
        ...
        btn_add_test_result = tk.Button(frame_add_test, text="Add Test Result", command=self.add_test_result, bg="gray", fg="white")
    ...
    def create_get_patient_info_frame(self):
        ...
        btn_get_patient_info = tk.Button(frame_get_patient_info, text="Get Info", command=self.get_patient_info, bg="blue",fg="white")
    ...
    def validate_gender(self, gender):
        self.entry_gender =   self.entry_gender.lower()
        return gender in ["Male".lower(),"Male", "Female".lower(),"Female","MALE","FEMALE"]
    ...
    def add_patient(self):
        ...
        if not self.validate_patient_id(patient_id):
            self.lbl_patient_status.config(text="Invalid Patient ID!", fg="red")
            return

        if not self.validate_name(name):
            self.lbl_patient_status.config(text="Invalid Name!", fg="red")
            return

        if not self.validate_age(age):
            self.lbl_patient_status.config(text="Invalid Age!", fg="red")
            return

        if not self.validate_gender(gender):
            self.lbl_patient_status.config(text="Invalid Gender! Use 'Male' or 'Female'.", fg="red")
            return
    ...
    def add_test_result(self):
        ...
        if not self.validate_patient_id(patient_id):
            self.lbl_test_status.config(text="Invalid Patient ID!", fg="red")
            return

        if not self.validate_test_name(test_name):
            self.lbl_test_status.config(text="Invalid Test Name!", fg="red")
            return

        if not self.validate_test_result(result):
            self.lbl_test_status.config(text="Invalid Test Result!", fg="red")
            return
    ...
    def get_patient_info(self):
        ...
        if not self.validate_patient_id(patient_id):
            self.lbl_patient_info.config(text="Invalid Patient ID!", fg="red")
            return
